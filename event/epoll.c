...
int iowatcher_init(hloop_t* loop) {
    ...
    epoll_ctx->epfd = (int)epoll_create(EVENTS_INIT_SIZE);
...
int iowatcher_cleanup(hloop_t* loop) {
    ...
    epoll_close((HANDLE)epoll_ctx->epfd);
...
int iowatcher_add_event(hloop_t* loop, int fd, int events) {
    ...
    epoll_ctl((HANDLE)epoll_ctx->epfd, op, fd, &ee);
    if (op == EPOLL_CTL_ADD) {
...
int iowatcher_poll_events(hloop_t* loop, int timeout) {
    ...
    int nepoll = epoll_wait((HANDLE)epoll_ctx->epfd, epoll_ctx->events.ptr, epoll_ctx->events.size, timeout);
    if (nepoll < 0) {
        if (errno == EINTR) {
            return 0;
        }
